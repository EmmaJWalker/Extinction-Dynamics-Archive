...
############################################################################################
destroy.vs.degrade<-function(landscape, a, delta, landscape.type){
  ...
  for(j in 1:(n.patches/10-1)) { #until all but 10 patches has been removed
    print(j)
    w<-subset(r.landscape, rowSums(is.na(r.landscape)) != (3+n.patches))
    q<-c(colSums(is.na(w)) != (n.patches-10*j+10))
    w<-subset(w, select=q)
    ...
    print(eq.p.r[j])
    avg.p.r[j]<-sum(p.star.r)/(n.patches-10*j+10) #average predicted probability of occupancy across patches at eq
    print(avg.p.r[j])
    ...
    #print(time.to.eq.r[j])
    time.to.p1000.r[j]<-SRLM.output.r$time.to.p1000 #timestep at which the metapopulation reached what it's average occupancy 
    #print(time.to.p1000.r[j])
    #for the last 50 of 1000 timesteps would be
    sim.eq.size.r[j]<-SRLM.output.r$eq.size/(n.patches-10*j+10) #average number of patches that were occupied on average for the 
    #print(sim.eq.size.r[j])
    ...
    percent.habitatloss[j]<-prev.loss+sum(r.landscape$A[r])/total.area
    print(percent.habitatloss[j])
