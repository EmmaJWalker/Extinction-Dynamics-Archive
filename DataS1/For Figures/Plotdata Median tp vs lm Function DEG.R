...
##############################################################################################################
plotdata.median.tp.vs.lm.e<-function(input.data){
  input.data<-input.data
  input.data$lambda.M.e<-input.data$lambda.M.e/20
#input.data$binned.lambda.M.r <- cut(input.data$lambda.M.r, c(seq(0, 1, 0.1),Inf), labels=seq(0,1,0.1))
  input.data$lambda.M.e <- input.data$lambda.M.e*2 
  input.data$binned.lambda.M.e <- round(input.data$lambda.M.e, 1)/2 #to get intervals of 0.5
  input.data$binned.lambda.M.e<-as.numeric(as.character(input.data$binned.lambda.M.e)) ############**************
  bins<-unique(input.data$binned.lambda.M.e) #################***************
  ...
  for (i in 1:length(bins)) {
    x<-subset(input.data, binned.lambda.M.e==bins[i])
    #destruction
